...
/**
  ...
  * @returns {number[]}
  */
export function add(...colors) {
  if (colors.length < 2) {
    throw new Error('Can\'t add less than 2 colors');
  }
  const result = colors[0].slice();
  for (let i = 0; i < 3; i++) {
...
/**
  ...
  * @returns {number[]}
  */
export function multiply(...colors) {
  if (colors.length < 2) {
    throw new Error('Can\'t multiply less than 2 colors');
  }
  const result = colors[0].slice();
  for (let i = 0; i < 3; i++) {
...
export function randomize(color, diff) {
  if (!(diff instanceof Array)) { diff = Math.round(getNormal(0, diff)); }
  const result = color.slice();
  for (let i = 0; i < 3; i++) {
    result[i] += (diff instanceof Array ? Math.round(getNormal(0, diff[i])) : diff);
  }
  return result;
}
...
export function fromString(str) {
  ...
  if (_cache.hasOwnProperty(str)) {
    ...
    } else if ((r = str.match(/rgb\(([0-9, ]+)\)/i))) { /* decimal rgb */ // eslint-disable-line no-cond-assign
      cached = r[1].split(/\s*,\s*/).map(x => parseInt(x, 10));
    } else { /* html name */
      cached = [0, 0, 0];
    }
