...
ROT.Map.Uniform.prototype._generateRoom = function () {
  ...
  while (count < this._roomAttempts) {
    ...
    const room = ROT.Map.Feature.Room.createRandom(this._width, this._height, this._options);
...
ROT.Map.Uniform.prototype._digLine = function (points) {
  for (let i = 1; i < points.length; i++) {
    ...
    const end = points[i];
    const corridor = new ROT.Map.Feature.Corridor(start[0], start[1], end[0], end[1]);
