...
ROT.Map.Feature.Room.createRandomCenter = function (cx, cy, options) {
  ...
  const x1 = cx - Math.floor(ROT.RNG.getUniform() * width);
  const y1 = cy - Math.floor(ROT.RNG.getUniform() * height);
  const x2 = x1 + width - 1;
...
ROT.Map.Feature.Room.createRandom = function (availWidth, availHeight, options) {
  ...
  const x1 = 1 + Math.floor(ROT.RNG.getUniform() * left);
  const y1 = 1 + Math.floor(ROT.RNG.getUniform() * top);
  const x2 = x1 + width - 1;
  const y2 = y1 + height - 1;
...
ROT.Map.Feature.Corridor.createRandomAt = function (x, y, dx, dy, options) {
  ...
  const length = ROT.RNG.getUniformInt(min, max);
